...
class RSSimpleFPN(BaseModule):
    def __init__(self,
        ...
        self.fpn1 = nn.Sequential(
            ...
            nn.LayerNorm((640, 128, 128)),
            # nn.LayerNorm((self.backbone_channel // 2,80,80)),
            nn.GELU(),
            nn.ConvTranspose2d(self.backbone_channel // 2,self.backbone_channel // 4, 2, 2),
            nn.Conv2d(self.backbone_channel // 4, out_channels, kernel_size=1),
            # build_norm_layer(norm_cfg, self.backbone_channel // 4)[1],
            nn.LayerNorm((256, 256, 256)),
            # nn.LayerNorm((256,160,160)),
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)             
            )
        self.fpn2 = nn.Sequential(
            nn.ConvTranspose2d(self.backbone_channel, self.backbone_channel // 2, 2, 2),
            nn.Conv2d(self.backbone_channel // 2, out_channels, kernel_size=1),
            # build_norm_layer(norm_cfg, self.backbone_channel // 2)[1],  
            nn.LayerNorm((256, 128, 128)),
            # nn.LayerNorm((256,80,80)),
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
            )
        self.fpn3 = nn.Sequential(nn.Identity(),
            nn.Conv2d(self.backbone_channel, out_channels, kernel_size=1),
            # build_norm_layer(norm_cfg, self.backbone_channel * 2)[1],
            nn.LayerNorm((256, 64, 64)),
            # nn.LayerNorm((256,40,40)),
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)                      
            )
        self.fpn4 = nn.Sequential(nn.MaxPool2d(kernel_size=2, stride=2),
            ...
            # nn.LayerNorm((256,20,20)),
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
            )
        self.fpn5 = nn.Sequential(nn.MaxPool2d(kernel_size=2, stride=2),
    ...
    def forward(self, input: Tensor) -> tuple:
        ...
        # assert len(input) == 33
        # input = input[-1]
        # input = [einops.rearrange(x, 'b h w c -> b c h w') for x in inputs]
        # build FPN
        inputs = []
        inputs.append(self.fpn1(input[7]))
        inputs.append(self.fpn2(input[13]))
        inputs.append(self.fpn3(input[19]))
        inputs.append(self.fpn4(input[25]))
        inputs.append(self.fpn5(input[-1]))
